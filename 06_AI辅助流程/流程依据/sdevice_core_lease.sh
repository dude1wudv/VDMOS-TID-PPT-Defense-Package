#!/usr/bin/env bash
# Atomic, fail-closed single-core SDevice lease manager.
# Derived from E:/VDMOS_TID_Research/scripts/sdevice_core_lease.sh.
set -euo pipefail

usage() { echo "usage: $0 {acquire|bind|release|status} [options]" >&2; exit 64; }
...
shift || true
lease_root=''; reserved_cores=''; max_managed_slots=''; lock_timeout_seconds=''
remote_run_dir=''; owner_pid=''; token=''; core=''
while [[ $# -gt 0 ]]; do
  case "$1" in
...
case "$command" in
  acquire)
    require_root "$lease_root"; require_core_list "$reserved_cores"
    require_uint max_managed_slots "$max_managed_slots"; require_uint lock_timeout_seconds "$lock_timeout_seconds"
    require_run_dir "$remote_run_dir"; require_uint owner_pid "$owner_pid"; (( owner_pid > 0 )) || fail invalid_owner_pid ;;
  bind)
...
range_contains() {
  ...
  for item in "${items[@]}"; do
    ...
    elif [[ "$item" == "$wanted" ]]; then return 0; fi
  done
  return 1
}
...
case "$command" in
  acquire)
    ...
    while IFS= read -r pid; do
      ...
      for ((c=0; c<online; c++)); do range_contains "$c" "$affinity" && blocked[$c]=unmanaged; done
    done < <(pgrep -x sdevice || true)
    selected=''
    for ((c=0; c<online; c++)); do [[ -z ${blocked[$c]+x} ]] && { selected=$c; break; }; done
    [[ -n "$selected" ]] || fail no_safe_core_available
    token=$(od -An -N16 -tx1 /dev/urandom | tr -d ' \n'); require_token "$token"
    file="$leases_dir/$token.lease"
    {
      printf 'core=%s\n' "$selected"; printf 'token=%s\n' "$token"
      printf 'owner_pid=%s\n' "$owner_pid"; printf 'remote_run_dir=%s\n' "$remote_run_dir"
      printf 'created_at=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    } > "$file"
    printf 'action=acquire\ncore=%s\ntoken=%s\nlease_file=%s\nowner_pid=%s\n' "$selected" "$token" "$file" "$owner_pid" ;;
  bind)
    file="$leases_dir/$token.lease"; [[ -f "$file" ]] || fail lease_not_found
    [[ "$(read_lease_value "$file" core || true)" == "$core" ]] || fail core_mismatch
    [[ "$(read_lease_value "$file" token || true)" == "$token" ]] || fail token_mismatch
    tmp="$file.tmp.$$"; sed '/^owner_pid=/d' "$file" > "$tmp"; printf 'owner_pid=%s\n' "$owner_pid" >> "$tmp"; mv -f -- "$tmp" "$file"
    printf 'action=bind\ncore=%s\ntoken=%s\nowner_pid=%s\n' "$core" "$token" "$owner_pid" ;;
  release)
    file="$leases_dir/$token.lease"; [[ -f "$file" ]] || fail lease_not_found
    [[ "$(read_lease_value "$file" core || true)" == "$core" ]] || fail core_mismatch
    [[ "$(read_lease_value "$file" token || true)" == "$token" ]] || fail token_mismatch
    rm -f -- "$file"; printf 'action=release\ncore=%s\ntoken=%s\nreleased=true\n' "$core" "$token" ;;
  status)
    cleanup_stale_leases; online=$(getconf _NPROCESSORS_ONLN) || fail online_core_probe_failed
    lease_count=0; shopt -s nullglob; for file in "$leases_dir"/*.lease; do lease_count=$((lease_count + 1)); done
